...
class ClarendoncountySpider(scrapy.Spider):
    name = 'clarendoncounty'
    start_urls = ['https://chamber.clarendoncounty.com/list']

    custom_settings = {
        'FEED_URI': 'clarendoncounty.csv',
        'FEED_FORMAT': 'csv',
    ...
    def detail_page(self, response):
        try:
            item = response.meta['item']
            item['Business Name'] = response.css('h2[itemprop="name"] a::text').get('').strip()
            ...
            item['Business_Site'] = response.css('li.gz-card-website a[itemprop="url"]::attr(href)').get('').strip()
            item['Social_Media'] = ', '.join(
                data.css('::attr(href)').get('') for data in response.css('li.gz-card-social a'))
            item['Source_URL'] = 'https://chamber.clarendoncounty.com/list'
            occupy = response.css('span.gz-cat::text').get('').strip()
            if occupy:
            ...
            item['Lead_Source'] = 'clarendoncounty'
